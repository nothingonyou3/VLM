...
def get_text_embedding(text, tokenizer):
...
class CRCDataset(Dataset):
    ...
    def __init__(self, split):
        ...
        self.data =  datasets.CRC(
                                ...
                                split=split,
                                download = download,
                                image_transforms = ResizeAndCrop(mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225]),
                            )
...
class CRCTDataModule(pl.LightningDataModule):
    def __init__(self, batch_size, num_workers):
        ...
        self.training_data = CRCDataset("train")
        self.test_data = CRCDataset("val")
